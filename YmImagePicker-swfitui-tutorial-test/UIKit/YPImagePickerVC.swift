...
class YPImagePickerVC: UIViewController {
    
    var selectedItems = [YPMediaItem]()
    
    var viewModel : YPImagePickerVM?
    ...
    let resultsButton = UIButton()
     ...
     init(_ viewModel: YPImagePickerVM) {
        super.init(nibName: nil, bundle: nil)
        print(#fileID, #function, #line, "viewModel.selectedItems.count : \(viewModel.selectedItems.count)")
    ...
    // 이것은 뷰가 로드 되었을 때 - 키워드: 라이프사이클
    // 라이프사이클이 뭐지? 나중에 검색해보자
...
//            self.selectedItems = items
            ...
            self.viewModel?.selectedItems = items
            picker.dismiss(animated: true, completion: nil)
...
// Support methods
extension YPImagePickerVC {
    
    func updateUIImageView(_ mediaItem: YPMediaItem){
...
extension YPImagePickerVC: YPImagePickerDelegate {
    func noPhotos() {}
...
extension YPImagePickerVC {
    fileprivate func observeVM(){
        ...
        viewModel?
            ...
            .sink{ firstItem in
                print(#fileID, #function, #line, "")
                self.updateUIImageView(firstItem)
            }.store(in: &subscriptions)
        
        viewModel?
            .$selectedItems
            .sink{ selectedItems in
                print(#fileID, #function, #line, "")
